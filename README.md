...
Ask a coding agent to study `masker.m`, implement `masker.py`, verify it against
the source, and report any deliberate platform adaptations. The result should be
one self-contained Python application with a minimalist, responsive GUI and a
short behavioural-equivalence report.

The repository includes a reference [`masker.py`](masker.py).
...
inventory, inspect it together, then implement and audit. This keeps the
vibe-coding pace while making assumptions visible. A pure one-shot is an
interesting speed comparison, but makes silent omissions more likely. The prompt
below supports both: use it unchanged for one message, or ask the agent to pause
after its inventory for a two-pass run.
...
If the first pass looks plausible but uncertain, follow with:
...
close/return/save behaviour. A plausible-looking window can still be a poor port,
so give the agent time to trace the callbacks and compare its result with MATLAB.
...
## Human testing found a real bug

Automated tests can check array logic and callback state, but they do not fully
test a GUI backend. During a human pass, pressing `s` opened Matplotlib's **Save
Figure** action instead of acting only as Masker's brush-size modifier. Masker's
callback was present, but Matplotlib also reserves `s` as a default shortcut.
...
appears. This small conflict is exactly the kind of issue a short human-in-the-loop
pass can catch after automated checks succeed.
